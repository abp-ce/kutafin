...
def check_iframe(driver, place):
    """
    Закрывает ws-quiz-ifame, если он есть. Возвращает True,
    если ws-quiz-iframe обработан.
    """
...
def selenium_frigate(tables, user='dockeep9@gmail.com',
                     password='l4}$04|G') -> str:
    """
    Реализует двухвакторную авторизацию и возвращает массив HTML таблиц
    (<table>...</table>) для запрашиваемых таблиц в параметре tables
    (массив строк)
    """
    options = Options()
    ...
    if try_exc(driver, 'xpath', '//a[@href="come-in"]', 'Login') is None:
        return None

    if try_exc(
        driver,
        'xpath',
        ('//div[@id="js_come-in"]/div[@class="popup_content"]'
         '/form[@class="popup-form"]/'
         'div[@class="inp-w"]/input[@name="email"]'),
        'User',
        user
    ) is None:
        return None

    if try_exc(driver, 'name', 'password', 'Password', password) is None:
        return None

    ifrm_value = 'iframe[src^="https://www.google.com/recaptcha/api2/anchor"]'
    ...
    if try_exc(
        ...
        'xpath',
        ('//div[@id="js_come-in"]/div[@class="popup_content"]'
         '/form[@class="popup-form"]/button[1]'),
        'Submit 1'
    ) is None:
        return None
    ...
    if try_exc(
        driver,
        'xpath',
        '//div[@class="forgot-pass-w"]/form[1]/button[1]',
        'Submit 2'
    ) is None:
        return None
